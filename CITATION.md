...
Ernest Opoku. *Nondiagonal PyEPT*, version 0.2.0. Python software.
...
targets, convergence tolerance, and the nD-NRL3 formulation/static_space setting.
A version number alone does not distinguish updates made on the main branch.
...
Cite the parent NRL3 paper and this software's exact commit. Record the
formulation: version 0.2.0 defaults to `sector-projected-v2`
(`static_space="sector"`), while `static_space="full"` reproduces
`full-space-v1`. They are different approximations and must not be mixed in
benchmark or manuscript tables. The [working equations](docs/NON_DYSON_NRL3.md)
and [cross-basis validation](docs/NON_DYSON_SECTOR_VALIDATION.md) define this
project-specific extension; the non-Dyson ADC papers do not establish its
working equations or accuracy.
